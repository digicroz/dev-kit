...
async function createInitialVSCodeSettings(generatorsConfig?: GeneratorsConfig): Promise<void> {
...
async function detectAndConfigureSpringBootServices(): Promise<SpringBootConfig | undefined> {
  try {
      ...
      if (includeService) {
        const { startOrder } = await inquirer.prompt({
          ...
          validate: (input: number | undefined) =>
            (input !== undefined && input >= 0) || 'Starting order must be 0 or greater',
        });
...
async function configureGenerators(
  ...
  if (projectType === 'nextjs') {
    const { configurePublicDir } = await inquirer.prompt({
      ...
      message: 'Would you like to configure a public directory for static assets?',
      default: true,
    });
