...
import { existsSync, readFileSync, mkdirSync, readdirSync, statSync, createReadStream } from 'fs';
...
export function validateDatabaseSupport(): {
  ...
  if (!config) {
    ui.error('No dk.config.json found', "Run 'dk init' to initialize your project configuration");
...
export async function dbStatus(): Promise<void> {
  ...
  ui.info('Database drivers status', 'All database drivers are bundled with DK CLI');
  ...
  try {
    ...
    const connectionResult = await testDatabaseConnection(dbConfig.dbType!, dbUrl);
    ...
    if (connectionResult.success) {
      ...
      ui.table([
        ...
        },
        {
          ...
          value: envResult.loaded ? chalk.green('Loaded') : chalk.yellow('Not found'),
        },
        ...
        ...(connectionResult.details
          ? [
              ...
              {
                key: 'Port',
                value: chalk.gray(connectionResult.details.port?.toString() || 'Default'),
              },
            ]
          : []),
      ]);
      ...
      console.log('');
      ui.error('Connection Error:', connectionResult.error || 'Unknown error occurred');
...
export async function dbDumpCreate(): Promise<void> {
  ...
  try {
    ...
    const timestamp = new Date().toISOString().replace(/[-:]/g, '').replace('T', '_').slice(0, 15); // Format: YYYYMMDD_HHMMSS
...
export async function dbDumpApply(options: { version?: string } = {}): Promise<void> {
  ...
  try {
    ...
    if (options.version) {
      ...
      ui.info('Version specified', `Using dump version v${selectedDump.version}`);
    } else {
...
export async function dbDropAllTables(options: { force?: boolean } = {}): Promise<void> {
  ui.section('⚠️ Database Drop All Tables', 'WARNING: This will delete ALL tables');
  ...
  try {
    ...
    ui.error('⚠️ DANGER ZONE ⚠️', 'This operation will permanently delete ALL tables and data!');
    ...
    if (!options.force) {
      const { confirmed } = await inquirer.prompt({
        type: 'confirm',
        name: 'confirmed',
        message: chalk.red('Are you absolutely sure you want to drop ALL tables?'),
        default: false,
      });
      ...
      const { dbNameConfirm } = await inquirer.prompt({
        type: 'input',
        name: 'dbNameConfirm',
        message: chalk.red(`Type the database name "${dbParams.database}" to confirm:`),
      });
    ...
    const dropSpinner = ui.createSpinner('Dropping all tables... This may take a moment');
    dropSpinner.start();
    ...
    try {
      ...
      ui.table([
        ...
        {
          key: 'Status',
          value: chalk.green(remainingCount === 0 ? 'Database is now empty' : 'Some tables remain'),
        },
        { key: 'Timestamp', value: chalk.gray(new Date().toLocaleString()) },
      ]);
