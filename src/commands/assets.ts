...
async function renameImagesInDirectory(
  ...
  for (const filePath of allFiles) {
    ...
    if (fileName !== convertedName) {
      ...
      if (existsSync(newPath) && fileName.toLowerCase() !== convertedName.toLowerCase()) {
        throw new Error(
      ...
      if (fileName.toLowerCase() === convertedName.toLowerCase() && fileName !== convertedName) {
...
export const generateImageIndex = async (): Promise<void> => {
  ...
  const baseDirPath = path.resolve(path.join(assetsConfig.baseDir, imageConfig.baseDir));

  if (!existsSync(baseDirPath)) {
    ui.error('Images directory not found', `Directory ${baseDirPath} does not exist`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (config.projectType === 'nextjs' && assetsConfig.publicDir) {
      publicDirPath = path.resolve(path.join(assetsConfig.publicDir, imageConfig.baseDir));
    ...
    const baseFileNames = new Set(
      baseFiles.map((f) => {
        ...
        return path.posix.normalize(relPath.split(path.sep).join(path.posix.sep));
      }),
    );
    ...
    const duplicates = [...baseFileNames].filter((name) => publicFileNames.has(name));
    ...
    for (const abs of baseFiles) {
      ...
      const baseNoExt = path.basename(relFromImages, path.extname(relFromImages));
      ...
      if (config.projectType === 'react-native-cli') {
        ...
        const varBase = toValidIdentifier(
          [...dirParts, baseNoExt].map(toCamel).filter(Boolean).join('_') || toCamel(baseNoExt),
        );
