...
export async function doctor(): Promise<void> {
  ui.section('🩺 DK Health Diagnostics', 'Comprehensive system and configuration check');
  ...
  try {
    ...
    if (availableCommands.length === dkCommands.length) {
      ...
      checks.push({
        name: 'DK Commands',
        status: 'fail',
        message: `Some DK commands are missing: ${dkCommands
  ...
  ui.info('Diagnostic Results:', 'Analysis of your DK installation and environment');
  console.log('');
  ...
  ui.table([
    ...
    {
      key: 'Warnings',
      value: chalk.yellow(checks.filter((c) => c.status === 'warn').length.toString()),
    },
    {
      key: 'Failures',
      value: chalk.red(checks.filter((c) => c.status === 'fail').length.toString()),
    },
  ]);
  ...
  if (!hasErrors && !hasWarnings) {
    ui.confirmBox('🎉 DK is working perfectly! All features are ready to use.', 'success');
    console.log(chalk.green('You can now use all DK commands:'));
    console.log(
      chalk.gray('  • ') + chalk.cyan('dk clean  ') + chalk.gray('- Clean temporary files'),
    );
    console.log(
      chalk.gray('  • ') + chalk.cyan('dk deploy ') + chalk.gray('- Deploy your project'),
    );
  } else if (!hasErrors && hasWarnings) {
    ui.confirmBox('⚠️ DK is working but some features may be limited.', 'warning');
    ...
    console.log(chalk.gray('  ') + chalk.cyan('npm install -g @digicroz/dev-kit'));
  }
  ...
  ui.confirmBox(
    '💡 Need Help?\n\n' +
      '📖 Documentation: https://github.com/digicroz/dev-kit\n' +
      '🐛 Report Issues: https://github.com/digicroz/dev-kit/issues\n' +
